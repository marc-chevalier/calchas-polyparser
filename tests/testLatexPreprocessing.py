from calchas_polyparser.latexPreprocessing import preprocess_implicit_braces
...
class TestLatexPreprocessing(TestCase):
    ...
    def testPreprocessingImplicitBraces(self):
        test_list = [("((abc)a^nb\\%b!+c)", "( ( abc ) a ^ {n}b \\% b ! + c ) "),
                     ("(abc)a^\\nb\\%b!+c", "( abc ) a ^ {\\nb} \\% b ! + c "),
                     ("f(x)a", "f ( x ) a "),
                     ("g f(n)", "g f ( n ) "),
        ...
        for (expr, res) in test_list:
            self.assertEqual(preprocess_implicit_braces(expr), res)
